...
async def main():
    if os.path.exists(f"currency_data/end_day_data_currency_{datetime.today().date()}.csv"):
        with open(f"currency_data/end_day_data_currency_{datetime.today().date()}.csv", "r") as json_file:
            end_day_data_file = json.load(json_file)
            await send_tg_message(end_day_data_file, common_dates)
